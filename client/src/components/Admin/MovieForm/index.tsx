...
const MovieForm = ({ values }: MovieFormProps) => {
  ...
  const getData = async () => {
    try {
      const data = await getDataForAddingMovie();
      if (data) {
  ...
  return (
    <div className="movie-form">
      <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
        <Fab onClick={refreshData} size="small">
          <Refresh />
        </Fab>
      </Box>
        ...
        <form onSubmit={methods.handleSubmit(onSubmit)}>
          <div className="movie-form-container">
            <div className="movie-form-wrapper">
              ...
              <CustomInput control={methods.control} name="year" label="Year" />
              <CustomInput
                control={methods.control}
                name="runtimeStr"
                label="Runtime"
              />
              <TextArea control={methods.control} name="plot" label="Plot" />
            </div>
            ...
            <div className="movie-form-wrapper">
              ...
              <Link to={"/admin/people"} target="_blank">
                <Fab size="small" variant="extended" sx={{ margin: 1 }}>
                  <Add /> Person
                </Fab>
              </Link>
